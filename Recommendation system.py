...
def recommend(movie):
    ...
    filtered_movies = newmoviedf[newmoviedf['title'] == movie]
    
    if not filtered_movies.empty:  # Check if any movie is found
        movie_index = filtered_movies.index[0]  # Fetching the index of that movie
...
pickle.dump(newmoviedf,open('artificats/movie_list.pk','wb'))
pickle.dump(similarity,open('artificats/movie_list.pk','wb'))
